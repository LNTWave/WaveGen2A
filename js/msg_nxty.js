...
// Version 1.0 protocol additions...
const   NXTY_SYS_INFO_REQ                 = 0x01;   // Get SelParamReg
const   NXTY_SYS_INFO_RSP                 = 0x41;
const     NXTY_SEL_PARAM_REG_CU_UID_TYPE  = 0x01;
const     NXTY_SEL_PARAM_REG_NU_UID_TYPE  = 0x02;
...
const     NXTY_SUPER_MSG_GET_REG_SUPPORT_DATA   = 0x15;
...
var cldSwVerNuCf           = null;      // nxtySwVerNuCf returned in egress from cloud. 
...
var nxty = {
    ...
    SendNxtyMsg: function( uCmdByte, pMsgData, uLenByte )
    {
      ...
      if( (uCmdByte == NXTY_DOWNLOAD_START_REQ)         || 
          ...
          (uCmdByte == NXTY_SYS_INFO_REQ)               || 
          (uCmdByte == NXTY_SW_VERSION_REQ)             || 
          (uCmdByte == NXTY_CELL_INFO_REQ)              || 
      ...
      else if( uCmdByte == NXTY_REGISTRATION_REQ )                     // V1 message 
      {
            // Create a new array that is initialized to all zeros...              
    ...
    ProcessNxtyRxMsg: function( pRxMsgData, uLenByte )
    {
        ...
        switch( uCmd )
        {
            ...
            {   
                ...
                nxtyRxBtCnx = u8RxBuff[2];      // Set to 1 if connected.
...
//                msgRxLastCmd = NXTY_WAITING_FOR_RSP;
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////
            //
            // ICD version 1.0 additions.....ICD 0.7:  
            //   Added back in for backward compatibility.  
            //   Fill in response data as if received by V2 message... 
            //
            case NXTY_SYS_INFO_RSP: 
            {
               PrintLog(1,  "V1 Msg: System info Rsp" );
...
//mySn = "IosTest";                
               ...
               else if( nxtyCurrentReq == NXTY_SEL_PARAM_REG_CU_UID_TYPE )
               {
                   ...
                   for( i = 0; i < 8; i++ )
                   {
                        ...
                        nxtyCuUniqueId += U8ToHexText(u8RxBuff[2+i]);
                   }
               }
               else if( nxtyCurrentReq == NXTY_SEL_PARAM_REG_NU_UID_TYPE )
               {
                   // SelParamReg_1 requested...
                   nxtyNuUniqueId = "0x";
                   for( i = 0; i < 8; i++ )
                   {
                        u8UniqueId[i] = u8RxBuff[2+i];
                        nxtyNuUniqueId += U8ToHexText(u8RxBuff[2+i]);
                   }
               }
...
// NO_U                   UpdateUniiIcon(bUniiUp);
               ...
               else
               {
                   ...
                   if( nxtyCurrentReq == NXTY_SEL_PARAM_REG_SUPPORT_DATA )
                   {
                       nxtyRegSupportData = nxtySelParamRegOneRsp;
                       PrintLog(1,  "  RegSupportData = 0x" + nxtyRegSupportData.toString(16) );
                   }
                   
                   // Simulate V2 super message response complete.
...
//                  myPlmnid       = myData.plmnid;
            ...
            case NXTY_REGISTRATION_RSP:
            {
                ...
                guiRegistrationLockBits = nxtyRxRegLockStatus = u8RxBuff[2];
                break;
            }
...
// NO_U                   UpdateUniiIcon(true);
                   ...
                   guiRegistrationLockBits = nxtyRxRegLockStatus = u8RxBuff[10];
                   ...
                   if( nxtyRxRegLockStatus )
                   {
                       ...
                       PrintLog(1,  "V1 Msg: SW Version Rsp: NU Ver=" + nxtySwVerNuCf + " BuildID=" + nxtySwBuildIdNu + " RegLock=0x" + U8ToHexText(nxtyRxRegLockStatus) + " (" + rLock + ")");               
                   }
                   else
                   {
                       PrintLog(1,  "V1 Msg: SW Version Rsp: NU Ver=" + nxtySwVerNuCf + " BuildID=" + nxtySwBuildIdNu + " RegLock=0x" + U8ToHexText(nxtyRxRegLockStatus));
                   }
               }
               else if( nxtyCurrentReq == NXTY_SW_CF_CU_TYPE )
               {
                   ...
                   PrintLog(1,  "V1 Msg: SW Version Rsp: CU Ver=" + nxtySwVerCuCf + " BuildID=" + nxtySwBuildIdCu);
               }
               ...
               else if( nxtyCurrentReq == NXTY_SW_CU_PIC_TYPE )
               {
                   ...
                   PrintLog(1,  "V1 Msg: SW Version Rsp: CU PIC Ver=" + nxtySwVerCuPic);
               ...
               {
                    // 0xx.0yy
                    nxtySwVerCuBt    = "0" + U8ToHexText(u8RxBuff[4]) + "." + "0" + U8ToHexText(u8RxBuff[5]); 
                    PrintLog(1,  "Msg: SW Version Rsp: BT Ver=" + nxtySwVerCuBt);
                   ...
                   if( (u8RxBuff[4] == 0xFF) && (u8RxBuff[5] == 0xFF) )
                   {
                      ...
                      PrintLog(1,  "Msg: SW Version Rsp: BT: Changed to Ver=" + nxtySwVerCuBt);
                   }
            ...
            {
                PrintLog(2,  "Msg: Read Address Rsp: Value=0x" + U8ToHexText(u8RxBuff[4]) + U8ToHexText(u8RxBuff[5]) + U8ToHexText(u8RxBuff[6]) + U8ToHexText(u8RxBuff[7]) );
...
//               nxtyReadAddrRsp >>>= 0; 
            ...
            {
               PrintLog(2,  "Msg: Write Address Rsp: (1=pass) status=0x" + U8ToHexText(u8RxBuff[4]) );
            ...
            case NXTY_READ_DATA_RSP:
            {
                ...
                {
                    PrintLog(2,  "Msg: Read Data Rsp: Tech Type" );
...
//                                myModel = "MN" + (nxtyRxStatusBoardConfig | 0x01);    // We now use the SKU as the Model number
                            ...
                            if( !(nxtyRxStatusBoardConfig & IM_A_CU_MASK) )
                            {
                                ...
                                if( (u8RxBuff[94] != 0x00) && (u8RxBuff[94] != 0xFF) && (u8RxBuff[101] != 0x00) && (u8RxBuff[101] != 0xFF) )
                                {
                                    ...
                                    //                      A       T        U        S   
                                    // Hash it..."ATUS" ==> 65 + (2*84) + (3*85) + (4*83) = 820 % 1000 = 820    (always less than 1000)
                                    var hashOp = (u8RxBuff[108] + (2*u8RxBuff[109]) + (3*u8RxBuff[110]) + (4*u8RxBuff[111])) % 1000; 
...
//                               nxtyRxRegLockStatus  = u8RxBuff[83];
                ...
                else if( nxtyCurrentReq == NXTY_SUPER_MSG_INFO2_TYPE )
                {
                    ...
                    if( (u8RxBuff[4]  == NXTY_NAK_RSP)  || (u8RxBuff[6] == NXTY_NAK_RSP)   || (u8RxBuff[11] == NXTY_NAK_RSP)  || (u8RxBuff[13] == NXTY_NAK_RSP) ||    // ART 
                        ...
                        (u8RxBuff[74] == NXTY_NAK_RSP)  || (u8RxBuff[76] == NXTY_NAK_RSP)  )                                                                          // Chan List
                    {
                        // Got a NAK...
                        iNxtySuperMsgRspStatus = NXTY_SUPER_MSG_STATUS_FAIL_NAK;
                    ...
                    else
                    {
                        ...
                        if( (u8RxBuff[5]  == 1)  &&  (u8RxBuff[12] == 1)  &&  (u8RxBuff[19] == 1)  &&  (u8RxBuff[26] == 1)  &&     // ART
                            ...
                            (u8RxBuff[61] == 1)  &&  (u8RxBuff[68] == 1)  &&  (u8RxBuff[75] == 1)                                                  ) 
                        {
                            iNxtySuperMsgRspStatus = NXTY_SUPER_MSG_STATUS_SUCCESS;
                            ...
                            // RegSupport Cell-Search CU only or one-box ...............................................
                            if( (nxtyRxStatusBoardConfig & IM_A_CU_MASK) || (bCnxToOneBoxNu) )
                            {
                                nxtyRegSupportData   =     (u8RxBuff[77] << 24) |          
                                                           (u8RxBuff[78] << 16) |          
                                                           (u8RxBuff[79] << 8)  |        
                                                            u8RxBuff[80];

                                nxtyRegSupportData >>>= 0;
                                PrintLog(1,  "  RegSupportData = 0x" + nxtyRegSupportData.toString(16) );
                            }
                ...
                else if( nxtyCurrentReq == NXTY_SUPER_MSG_GET_BOARD_CONFIG )
                {
                    ...
                    else
                    {
                        ...
                        var outText = "Super Msg Rsp: Board Config: UART Redirected to: ";
                        outText += (nxtyRxStatusBoardConfig & IM_A_CU_MASK)?"CU":"NU";
                        PrintLog(1,  outText );
                    }
                }
                else if( nxtyCurrentReq == NXTY_SUPER_MSG_GET_REG_SUPPORT_DATA )
                {
                    // Read the Reg support data ...

                    //                   Write Reg Support     Read  
                    // Tx: ae 62 9d 13   11 f0 0 0 20 0 0 0 11 10 f0 0 0 1c  
                    // Rx  ae 31 ce 53   51 1                  50 0 0 0 7 
                    //     [0]           [4]                   [6] 
                    ...
                    if( (u8RxBuff[4]  == NXTY_NAK_RSP) || (u8RxBuff[6]  == NXTY_NAK_RSP) ) 
                    {
                        // Got a NAK...
                        iNxtySuperMsgRspStatus = NXTY_SUPER_MSG_STATUS_FAIL_NAK;
                        PrintLog(99,  "Super Msg: Get Reg support msg type encountered a NAK." );
                    }
                    else
                    {
                        iNxtySuperMsgRspStatus = NXTY_SUPER_MSG_STATUS_SUCCESS;
                        nxtyRegSupportData   =     (u8RxBuff[7] << 24) |          
                                                   (u8RxBuff[8] << 16) |          
                                                   (u8RxBuff[9] << 8)  |        
                                                    u8RxBuff[10];
...
{ 
    ...
    else if( u32Addr == NXTY_PCCTRL_UART_REDIRECT )
    {
        outText += "UART Redirect)  Go Local";
    }
     else
    {
...
{ 
    ...
    if( nxtyRxStatusIcd <= V1_ICD )
    {
        // Set the waiting flags in main to true so main loop continues....
        bNxtySuperMsgRsp  = true;
        return;
    }
    ...
    // Read Select Param Reg RegSupport for Cell-Search Info ...........................................                
    u8TempTxBuff[i++] = NXTY_WRITE_ADDRESS_REQ;
    u8TempTxBuff[i++] = (NXTY_PCCTRL_SELPARAM_REG >> 24);  
    u8TempTxBuff[i++] = (NXTY_PCCTRL_SELPARAM_REG >> 16);
    u8TempTxBuff[i++] = (NXTY_PCCTRL_SELPARAM_REG >> 8);
    ...
    u8TempTxBuff[i++] = 0x00;
    u8TempTxBuff[i++] = 0x00;
    u8TempTxBuff[i++] = NXTY_SEL_PARAM_REG_SUPPORT_DATA;                          
...
{ 
    ...
    {                        
        ...
        if( nxtyRxStatusIcd <= V1_ICD )
        {
            PrintLog(1,  "V1: Get Link State" );
        
            // Check to see if UNII is up...
            nxtyCurrentReq  = NXTY_SEL_PARAM_LINK_STATE;
            u8TempTxBuff[0] = 0x02;                       // Check CU   
            u8TempTxBuff[1] = NXTY_SEL_PARAM_LINK_STATE;  // SelParamReg 1: LinkState
            u8TempTxBuff[2] = NXTY_SEL_PARAM_LINK_STATE;  // SelParamReg 2: LinkState
            nxty.SendNxtyMsg(NXTY_SYS_INFO_REQ, u8TempTxBuff, 3); 
        }
        else
        {        
...
// GetSuperMsgRegSupportData .......................................................................................
function GetSuperMsgRegSupportData()
{
    var i            = 0;

    PrintLog(1,  "Super Msg Send: Get Reg Support Data." );
    ...
    nxtyCurrentReq = NXTY_SUPER_MSG_GET_REG_SUPPORT_DATA;
    nxty.SendNxtyMsg(NXTY_SUPER_MSG_REQ, u8TempTxBuff, i);
}
...
{ 
    if( bSouthBoundWriteError == true )
    {
        // No need to wait on response if we have a write error.
        msgRxLastCmd          = NXTY_INIT;
        bSouthBoundWriteError = false;
    }
