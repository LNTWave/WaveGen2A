...
function FileWaitNativeInit()
{
    if(QtFileSystemIf != null)
    {
        ...
        QtFileSystemIf.GetDownloadDir("", function(p){
            g_fileSystemDir = p;
            PrintLog(1, "  FileWaitNativeInit: filesystem access in directory: " + g_fileSystemDir);
        });
        PrintLog(10, "FileWaitNativeInit request filesystem path...");
    }
}
...
function ReadFile( myFileName ) 
{
    ...
    if(QtFileSystemIf != null)
    {
        QtFileSystemIf.doesFileExist(myFileName, function(bRet){
            if(true == bRet)
                g_fileReadSuccess = true;
            else
                g_fileReadSuccess = false;
            PrintLog(10, "  ReadFile returned " + g_fileReadSuccess);
        });
    }
}
...
//
//   Opens the default mail application to send mail
// 
//  Inputs: 
//           To        : TO field
//           Cc        : CC field
...
//
//
function OpenDefaultEmailClient( strTo, strCc, strBcc, strSubject, strBody)
{
PrintLog(10, "OpenDefaultEmailClient:{{" + strTo +"}}{{"+ strCc +"}}{{"+ strBcc +"}}{{"+ strSubject +"}}{{"+ strBody +"}}");
    if(QtFileSystemIf != null)
    {
        strBody = "*** NOTE: Please attach file '" + g_fileSystemDir + "Wave2Log.txt' *** \n\r" + strBody;
        QtFileSystemIf.SendEmail(strTo, strCc, strBcc, strSubject, strBody, 
            function(bRet)
            {
              PrintLog(10, "OpenDefaultEmailClient got return from QT:" + bRet);
            });
    }
}
