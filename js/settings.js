...
var bSettingsLoop = false;

// --------------------------------------------------------------------------------------------
function StartSettingsLoop(periodMs) 
{
    if( bSettingsLoop == false )
    {
        PrintLog(1, "StartSettingsLoop...");
        StgLoopIntervalHandle = setInterval(StgLoop, periodMs);
    }
...
// --------------------------------------------------------------------------------------------
function StopSettingsLoop() 
{
    if( bSettingsLoop == true )
    {
        PrintLog(1, "StopSettingsLoop...");
...
function updateAntStatus()
{
	...
	if(setAntennaCallFlag){
		ShowAlertPopUpMsg("Power Cycle", "power-cycle needed for changes to be applied.");
		setAntennaCallFlag = false;
	}
